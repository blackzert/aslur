    ...
    def syscallTrace(self):
        ...
        if syscall:
            if syscall.result is not None:
                ...
                #error(text)
                if self.inspector:
                    self.inspector.handle_syscall(syscall)
            else:
                text = syscall.format()
                if self.show_pid:
...
class Inspector():
    def __init__(self):
        self.open_files = {}
        self.syscalls = { "mmap": self.handle_mmap, "open": self.handle_open, "close": self.handle_close }

    def handle_open(self, filename, flags, mode, result):
        if result & 0xffffffffffffffff > 0:
            self.open_files[result] = filename

    def handle_mmap(self, addr, len, prot, flags, fd, pgoff, result):
        fname = ''
        if fd == 0xffffffffffffffff or fd == 0xffffffff:
            fname = "ANON"
        else:
            if fd in self.open_files:
                fname = self.open_files[fd]
            else:
                fname = str(fd)
        #print("MMAP( addr=0x%x, len=0x%x, prot=0x%x, flags=0x%x, fd=%s, pgoff=0x%x) = 0x%x"%(addr, len, prot, flags, fname, pgoff, result))
        print(fname)

    def handle_close(self, fd, result):
        if result == 0:
            if fd in self.open_files:
                del(self.open_files[fd])

    def handle_syscall(self, syscall):
        if not (syscall.name in self.syscalls):
            return
        h = self.syscalls[syscall.name]
        args = {}
